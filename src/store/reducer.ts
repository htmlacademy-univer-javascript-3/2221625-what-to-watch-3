...
const reducer = createReducer(initialState, (builder) => {
  builder
    .addCase(setGenre, (state, action) => {
      ...
      state.filmComps = filterFilmComps(payload, filmComps).slice(0,state.more);
    ...
    .addCase(setMore, (state, action) => {
      ...
      state.filmComps = payload ? filterFilmComps(state.genre, filmComps).slice(0,state.more) : filmComps;
    });
});
